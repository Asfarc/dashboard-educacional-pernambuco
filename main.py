...
def _musica_de_fundo(arquivo_mp3: str,
                     volume: float = 0.25,
                     flag: str = "musica_atual"): ...  # (mesma implementação)

def tocar_musica_sidebar(): ...  # (mesma implementação)

# chama logo após a configuração da página
tocar_musica_sidebar()

# ─── 3. ESTILO GLOBAL  ──────────────────────────────────────────────
CORES = { ... }  # (mesma definição de cores)

def css_global(c=CORES) -> str: ...  # (mesma implementação)
...
def aplicar_padrao_numerico_brasileiro(num): ...  # (mesma)

def format_number_br(num): ...  # (mesma)


# ─── 4‑B. PAGINAÇÃO ────────────────────────────────────────────────
class Paginator:
    def __init__(self, total, page_size=25, current=1): ...  # (mesma)
...
# E então a RAM e o restante dos filtros permanecem igual:
